...
| `!狀態` | 記憶存了多少、今天主動過幾次 |
| `!備份` | 把記憶檔傳給你存著（Discord 附件） |
| `!主動` | 不等排程，現在就叫他開口 |
| `!幫忙` | 指令表 |

**全形驚嘆號也認**（`！狀態` = `!狀態`）。中文輸入法打出來的預設就是全形，
不必為了下指令去切輸入法。全形空格（`！記住　某件事`）同樣會被正規化。
...
**對話越長，每則越貴。** 第一則只有人格加一句話，約 $0.02；聊到 60 則視窗塞滿之後，
每一次呼叫都要把整段歷史重送一遍，輸入那側會漲到十倍以上。

所以有兩個斷點在幫你擋：人格那塊、以及**最後一則他的話**。
斷點之前的東西（人格、摘要、整段歷史）下一輪原封不動，用一折的價格讀回來。
log 裡「快取讀」那個數字就是省下來的量——它應該隨著對話變長而變大。
...
`啟動文森特.bat` 直接點兩下就會跑，而且**掛了會自己爬起來**——
程式結束就等 10 秒重開，關掉視窗才是真的收工。

程式本身也會撐住：Discord 連線斷掉（`getaddrinfo failed`、Wi-Fi 換了、筆電睡醒）
不再讓整個程式死在那裡，而是 5 秒後重連，連續失敗就退避到最長 5 分鐘。
只有兩種情況會直接退出並告訴你原因——**token 不對**，或 **MESSAGE CONTENT
INTENT 沒開**。那兩個重連一萬次也沒用。
（裡面寫死 `python3.14`，你的直譯器不同就改那一行。）

記憶是**單一一個 SQLite 檔**：`discord-bot/data/vincent.db`。
機器壞了、資料夾刪了，就沒了。Discord 裡傳 `!備份` 他會把那個檔案傳給你，
存到雲端硬碟或隨便哪裡都好。要還原就把檔案放回原位。
...
|---|---|
| emoji 😭❤️ | 原樣，就在文字裡 |
| 自訂表情 `<:catcry:123>` | 轉成 `:catcry:`——名字就是意思 |
| **貼圖** | 「（她傳了貼圖：貓貓大哭）」。**只有名字，沒有畫面** |
| **圖片** | **真的看得到**。圖片直接附在那則訊息上送進模型 |
| 影片／PDF／其他檔案 | 只有檔名 |
| 回覆某一則 | 會附上被回覆那句的前 200 字 |
| 對他的訊息按表情回應 | **收不到**，目前沒有處理 |

圖片最多一次 4 張、單張上限 5 MB（`VINCENT_MAX_IMAGES` 可調，
`VINCENT_VISION=0` 可以整個關掉）。太大的會被略過，人格設定裡有交代他
「沒看到圖就問，不要瞎掰」。

圖只掛在她傳的那一輪，**不會存進記憶一路往後帶**——記憶裡只留
「（她傳了圖片：cat.png）」這行文字。省錢，也省 SQLite。
所以隔了幾十則之後再問「剛剛那張圖」，他是想不起來的。

> 貼圖那則以前是個 bug：貼圖不在 `message.content` 裡，整則訊息會被當成空的
> 直接丟掉——你傳貼圖給他，他不是不理你，是根本沒收到。
...
| 斷網、Wi-Fi 換掉、睡醒 | 程式自己重連（5 秒起，最長退避到 5 分鐘），不會死掉 |
